...
assert(baseModel.nativePayload.qButton?.foregroundStyle?.includes('qButtonUpForegroundStyle'), '导出模型应保留 26 键原始上划引用。');
...
assert(basePreviewPayload.qButtonForegroundStyle?.fontSize === 14, '预览 adapter 不应改变 26 字母真实字号。');
assert(basePreviewPayload.qButtonForegroundStyle?.center?.y === 0.54, '预览 adapter 应把默认 26 字母视觉中心映射为偏下。');
assert(basePreviewPayload.qButton?.foregroundStyle?.includes('qButtonSwipeUpForegroundStyle'), '预览 adapter 应把 26 原始上划引用映射为预览别名。');
...
assert(baseHtml.includes('font-size:21px'), '默认 26 键中文预览应放大显示字母，但不改变导出字号。');
...
assert(
  swipeForegroundCount(shiftQCellHtml) === swipeForegroundCount(previewKeyCellHtml(baseHtml, 'q')),
  'Shift 激活时 native 预览应保留 q 键上下划提示。',
);
assert(
...
assert(numericHtml.includes('grid-template-columns:0.7fr 1fr 1fr 1fr 0.7fr'), '数字 9 键预览左右功能列应等宽，并保持三列主数字键等宽。');
