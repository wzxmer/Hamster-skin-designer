...
- Jsonnet 随导出包输出：默认应用包不包含 `jsonnet/generated/`，`jsonnet/main.jsonnet` 固定导入 `core/build.libsonnet`，并保证 `jsonnet -S -m` 输出和直接 YAML 导出一致。
- 预设键盘只作为默认 seed，不作为功能边界；左侧模块的目标是覆盖所有可导出的皮肤能力。

## 当前工作焦点

- `apps/workbench` 当前继续打磨横竖屏预览、工具栏与键盘切换联动。
- 中文 `14 / 17 / 18 / 9` 已切到专用预览模板，当前剩余重点是继续收 `中文九键` 与数字键盘的整体几何关系、阴影和切换一致性。
...
- 图片类样式已接入同源链路：键盘背景、toolbar 背景、展开候选背景、长按气泡和长按选中背景由预览与 `SkinEffectModel` 共同解析；图片素材编辑尚未完整开放，左侧模块暂时禁用。
- 后续清理 seed 时，先判断字段是否属于布局、内容、样式、动作、候选、toolbar、素材或导出结构；属于用户可配置能力的，迁移到 schema 与左侧模块，而不是继续藏在 raw payload。
...
- 为缺失字段补可视化编辑模块。
- 维护 `docs/SEED_CUSTOMIZATION_COVERAGE.md`，按“预设 seed 字段 -> project.json 受控字段 -> 左侧模块 -> 预览/导出测试”推进覆盖。
