...
const alphabeticQButtonBlock = yamlBlock(alphabeticKeyboard, 'qButton');
...
assert(alphabeticQButtonBlock.includes('symbol: "q"') && !alphabeticQButtonBlock.includes('character: "q"'), '英文 26 键字母主按键导出应使用 symbol 类型。');
...
const customResourceProject = createSampleProject();
customResourceProject.meta.name = 'resource-skin';
customResourceProject.assets.resources.light.atlas = {
  source: 'data:image/png;base64,iVBORw0KGgo=',
  sprites: {
    IMG2: {
      rect: { x: 100, y: 0, width: 100, height: 100 },
      insets: { top: 8, right: 9, bottom: 10, left: 11 },
    },
  },
};
const customResourcePackage = buildSkinPackageFiles(customResourceProject);
assert(findFile(customResourcePackage, 'resource-skin/light/resources/atlas.png')?.content instanceof Uint8Array, '项目资源应导出为 resources png。');
const atlasYaml = findFile(customResourcePackage, 'resource-skin/light/resources/atlas.yaml')?.content || '';
assert(atlasYaml.includes('IMG2:') && atlasYaml.includes('width: 100') && atlasYaml.includes('left: 11'), '项目资源切片应导出为 resources yaml。');
